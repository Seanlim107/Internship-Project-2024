...
# Math.py referenced from https://github.com/skhadem/3D-BoundingBox
...
def create_corners(dimension, location=None, R=None, scale=2):
    dx = dimension[2] / scale
    dy = dimension[0] / scale
    dz = dimension[1] / scale
...
def calc_location(dimension, proj_matrix, box_2d, alpha, theta_ray, beta):
    #global orientation
    orient = alpha + theta_ray
    orient2 = beta
    R = rotation_matrix(orient, roll=beta)
...
def get_box_2d_from_detection(xyxy):
    x_min, y_min, x_max, y_max = xyxy  # Assuming the detection is (x_min, y_min, x_max, y_max)
